...
def solve():
    """
    Решение задачи E1: Смотрим на башни (простая версия)
    
    Задача:
    Дан массив из n целых чисел. Нужно найти количество подпоследовательностей,
    для которых множества видимых башен слева (L) и справа (R) совпадают
    с множествами для исходной последовательности.
    
    Входные данные:
    - Первая строка: количество тестовых случаев t (1 ≤ t ≤ 100)
    - Для каждого теста:
        * Первая строка: целое число n (1 ≤ n ≤ 5000)
        * Вторая строка: n целых чисел a1, a2, ..., an (1 ≤ ai ≤ 10^9)
      При этом сумма всех n по тестам ≤ 5000
    
    Выходные данные:
    - Для каждого теста выводится количество подходящих подпоследовательностей
      по модулю 998244353.
