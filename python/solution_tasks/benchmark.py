# -*- coding: utf-8 -*-
"""
Бенчмарк-скрипт для сравнения производительности CPython и PyPy.

Скрипт выполняет серию вычислительных тестов в трех режимах:
1. Однопоточное выполнение
2. Многопоточное выполнение (с GIL)
3. Многопроцессное выполнение (истинный параллелизм)

Основные тестовые задачи:
- fibonacci(n): Рекурсивный расчет чисел Фибоначчи (O(2^n))
- matrix_multiply(n): Умножение матриц (O(n^3))
- prime_sieve(n): Решето Эратосфена (O(n log log n))
- heavy_compute(n): Вычисления с плавающей точкой

Особенности реализации:
- Автоматическая настройка кодировки консоли для поддержки русского языка
- Точный замер времени с использованием time.perf_counter()
- Поддержка Windows/Linux/macOS
- Оптимизированные алгоритмы вычислений
- Четкое разделение тестовых случаев
- Корректная обработка прерываний и ошибок

Пример использования:
    python benchmark.py  # для CPython
    pypy benchmark.py    # для PyPy
"""
