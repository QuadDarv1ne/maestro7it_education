-- SOLUTION: Найти всех исполнителей, начинающихся на букву "B"
SELECT Name AS Исполнитель
FROM Artist
WHERE Name LIKE 'B%'
ORDER BY Name;

-- SOLUTION: Показать 5 самых дорогих треков каждого жанра
WITH TopTracks AS (
    SELECT 
        g.GenreId,
        g.Name AS Жанр,
...
-- SOLUTION: Найти клиентов, которые потратили более $40
SELECT 
...
-- SOLUTION: Показать месячную статистику продаж за 2021 год
SELECT 
...
-- SOLUTION: Найти треки, которые есть в более чем 10 плейлистах
SELECT 
    t.Name AS Трек,
...
-- SOLUTION: Сравнение продаж по странам
SELECT 
    c.Country AS Страна,
...
-- SOLUTION: Топ-10 альбомов по выручке
SELECT 
    a.Title AS Альбом,
...
-- SOLUTION: Клиенты по штатам/областям
SELECT 
    c.State AS Штат,
...
-- SOLUTION: Анализ продолжительности треков по жанрам
SELECT 
    g.Name AS Жанр,
...
-- SOLUTION: Клиенты, которые покупали в каждом году
SELECT 
    c.FirstName || ' ' || c.LastName AS Клиент,
