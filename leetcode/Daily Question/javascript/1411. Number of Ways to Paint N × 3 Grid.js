 ...
 * 
 * @param {number} n - количество строк в сетке
 * @return {number} - количество способов раскраски по модулю 10^9+7
 */
var numOfWays = function(n) {
    const MOD = 1000000007;
    
    // Инициализация для первой строки
    let a = 6;  // Строки типа "ABC" (3 разных цвета)
    let b = 6;  // Строки типа "ABA" (первый и третий цвета одинаковы)
    
    // Динамическое программирование для строк 2..n
    for (let i = 2; i <= n; i++) {
        const new_a = (2 * a + 2 * b) % MOD;
        const new_b = (2 * a + 3 * b) % MOD;
        a = new_a;
        b = new_b;
    }
