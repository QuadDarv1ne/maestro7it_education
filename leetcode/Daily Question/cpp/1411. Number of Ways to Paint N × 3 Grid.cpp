...
public:
    int numOfWays(int n) {
        ...
        long long a = 6;  // Строки типа "ABC" (3 разных цвета)
        long long b = 6;  // Строки типа "ABA" (первый и третий цвета одинаковы)
        
        // Динамическое программирование для строк 2..n
        for (int i = 2; i <= n; i++) {
            long long new_a = (2 * a + 2 * b) % MOD;
            long long new_b = (2 * a + 3 * b) % MOD;
            a = new_a;
            b = new_b;
        }
