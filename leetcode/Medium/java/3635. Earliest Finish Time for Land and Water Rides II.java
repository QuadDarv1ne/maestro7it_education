...
class Solution {
    ...
    private long solve(int[] firstStart, int[] firstDur,
        ...
        Arrays.fill(prefMinDur, Long.MAX_VALUE);
        for (int i = 0; i < m; i++) {
        ...
        Arrays.fill(suffMinFinish, Long.MAX_VALUE);
        for (int i = m - 1; i >= 0; i--) {
            long finish = (long) second[i][0] + second[i][1];
        ...
        for (int i = 0; i < n; i++) {
            ...
            int idx = lowerBound(starts, (int) firstEnd);
    ...
    /**
     * Бинарный поиск: находит первый индекс, где arr[index] >= target.
     * 
     * @param arr Отсортированный массив
     * @param target Искомое значение
     * @return Индекс первого элемента >= target
     */
