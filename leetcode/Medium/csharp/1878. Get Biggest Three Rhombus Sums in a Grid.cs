...
public class Solution {
    public int[] GetBiggestThree(int[][] grid) {
        ...
        SortedSet<int> sums = new SortedSet<int>(); // Отсортированный набор уникальных значений

        for (int i = 0; i < m; i++) {
            for (int j = 0; j < n; j++) {
                // Ромб размера 1
                sums.Add(grid[i][j]);
                if (sums.Count > 3) sums.Remove(sums.Min);
        ...
        // Преобразуем в массив в порядке убывания
        int[] result = new int[sums.Count];
        sums.CopyTo(result);
