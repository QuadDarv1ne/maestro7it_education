 ...
 *    - `k <= sqrt(n)` (малый шаг) — используем структуру разностных массивов с TreeMap для оптимизации.
 * 2. Для малых `k` аккумулируем мультипликаторы в diff и применяем их ко всем соответствующим элементам массива.
 * 3. После обработки всех запросов вычисляем XOR всех элементов массива.
 *
 * Временная сложность: O(n * sqrt(n) + Q), где n — размер массива, Q — количество запросов.
 * Пространственная сложность: O(n) для хранения дополнительных структур данных.
 *
