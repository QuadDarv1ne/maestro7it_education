...
def show_menu():
    ...
    print()
    print("  6. 🔗 WebSocket сервер (мультиплеер)")
    print("     • Реальное время онлайн игры")
    print("     • Автоматический поиск игроков")
    print("     • Чат во время игры")
    print("     • Система рейтинга")
    print()
    print("  7. 🌐 Сетевой сервер (расширенный)")
    print("     • Профессиональный матчмейкинг")
    print("     • Лобби и комнаты")
    print("     • Расширенная статистика")
    print("     • Поддержка турниров")
    print()
    print("  8. 🚪 Выход")
    print()
    print("-" * 50)
...
def run_network_server():
    """Запуск расширенного сетевого сервера"""
    print("🌐 Запуск расширенного сетевого сервера...")
    print()
    
    try:
        # Проверка зависимостей
        deps = check_dependencies(['websockets'])
        if not deps['websockets']:
            print("⚠️  Библиотека websockets не установлена!")
            choice = input("Установить websockets? (y/n): ").strip().lower()
            if choice == 'y':
                if not install_package('websockets'):
                    print("❌ Не удалось установить websockets")
                    input("Нажмите Enter для возврата в меню...")
                    return
            else:
                print("Для сетевого сервера нужна библиотека websockets!")
                input("Нажмите Enter для возврата в меню...")
                return
        
        # Запуск сервера
        server_script = Path(__file__).parent / "network_chess_server.py"
        if not server_script.exists():
            print("❌ Файл сервера network_chess_server.py не найден!")
            input("Нажмите Enter для возврата в меню...")
            return
            
        print("✅ Расширенный сервер готов к запуску")
        print("🌐 Сервер будет доступен по адресу: ws://localhost:8766")
        print("🎮 Используйте network_chess_client.html для подключения")
        print("🏆 Поддержка рейтинга и турниров")
        print("=" * 50)
        print()
        ...
        print(f"❌ Ошибка запуска сетевого сервера: {e}")
        input("Нажмите Enter для возврата в меню...")
...
def main():
    ...
    while True:
        ...
        try:
            choice = input("Введите номер варианта (1-8): ").strip()
