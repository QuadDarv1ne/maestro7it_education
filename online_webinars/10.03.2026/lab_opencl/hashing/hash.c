...
const char* get_embedded_kernel() {
    ...
    "uint blk=0;uint datalen=len;\n"
    "while(len>0||blk==0){\n"
    "for(int i=0;i<64;i++)w[i]=0;\n"
    "uint copylen=len>64?64:len;\n"
    "for(uint i=0;i<copylen;i++){uint wi=i>>2;uint bi=3-(i&3);w[wi]|=((uint)input[offset+blk*64+i])<<(bi<<3);}\n"
    "uint padpos=blk*64+copylen;\n"
    "if(copylen<64){uint wi=copylen>>2;uint bi=3-(copylen&3);w[wi]|=((uint)0x80)<<(bi<<3);if(copylen>=56){for(int i=16;i<64;i++)w[i]=SIG1(w[i-2])+w[i-7]+SIG0(w[i-15])+w[i-16];uint a=h[0],b=h[1],c=h[2],d=h[3],e=h[4],f=h[5],g=h[6],hv=h[7];for(int i=0;i<64;i++){uint t1=hv+EP1(e)+CH(e,f,g)+SHA256_K[i]+w[i];uint t2=EP0(a)+MAJ(a,b,c);hv=g;g=f;f=e;e=d+t1;d=c;c=b;b=a;a=t1+t2;}h[0]+=a;h[1]+=b;h[2]+=c;h[3]+=d;h[4]+=e;h[5]+=f;h[6]+=g;h[7]+=hv;for(int i=0;i<64;i++)w[i]=0;w[0]=0x80000000;}}\n"
    "if(blk*64+64>=datalen){ulong bl=(ulong)datalen<<3;w[14]=(uint)(bl>>32);w[15]=(uint)bl;}\n"
