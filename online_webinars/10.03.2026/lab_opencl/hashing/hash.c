...
void print_device_info(cl_device_id device) {
    ...
    cl_uint compute_units, max_wg_size;
...
const char* get_embedded_kernel() {
    ...
    "for(int i=0;i<16;i++)w[i]=0;for(uint i=0;i<len&&i<56;i++){uint word_idx=i/4;uint byte_idx=3-(i%4);w[word_idx]|=((uint)input[offset+i])<<(byte_idx*8);}\n"
    "uint pad_pos=len;if(pad_pos<56){uint word_idx=pad_pos/4;uint byte_idx=3-(pad_pos%4);w[word_idx]|=((uint)0x80)<<(byte_idx*8);}\n"
    "w[14]=0;w[15]=len*8;for(int i=16;i<64;i++)w[i]=SIG1(w[i-2])+w[i-7]+SIG0(w[i-15])+w[i-16];\n"
    "uint a=h[0],b=h[1],c=h[2],d=h[3],e=h[4],f=h[5],g=h[6],h_val=h[7];\n"
    "for(int i=0;i<64;i++){uint t1=h_val+EP1(e)+CH(e,f,g)+SHA256_K[i]+w[i];uint t2=EP0(a)+MAJ(a,b,c);h_val=g;g=f;f=e;e=d+t1;d=c;c=b;b=a;a=t1+t2;}\n"
    "h[0]+=a;h[1]+=b;h[2]+=c;h[3]+=d;h[4]+=e;h[5]+=f;h[6]+=g;h[7]+=h_val;\n"
    "for(int i=0;i<8;i++){output[gid*32+i*4+0]=(h[i]>>24)&0xFF;output[gid*32+i*4+1]=(h[i]>>16)&0xFF;output[gid*32+i*4+2]=(h[i]>>8)&0xFF;output[gid*32+i*4+3]=h[i]&0xFF;}}\n"
