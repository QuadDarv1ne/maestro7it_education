...
Данная база данных предназначена для хранения информации о животных, обитающих в зоопарке.

Содержит данные о видах, характеристиках, ареалах обитания, условиях содержания, кормлении, медицинских записях и особенностях размножения животных. Также включает информацию о сотрудниках, вольерах и вакцинации.

## 🗄️ Структура базы данных

База данных состоит из следующих файлов, организованных по папкам:

### schema/
- `01_database_schema.sql` - создание структуры базы данных (таблицы, связи)
...
### analytics/
- `advanced_analytics.sql` - расширенная аналитика и бизнес-отчеты

### security/
- `security_enhancements.sql` - улучшения безопасности, роли и аудит

### utilities/
- `analytics_system.sql` - система аналитики и отчетности
- `auth_system.sql` - система аутентификации и авторизации пользователей
- `backup_system.sql` - система резервного копирования
...
2. Создайте базу данных, выполнив скрипт `schema/01_database_schema.sql`
3. Наполните базу данными из файла `data/02_sample_data.sql`
4. Создайте представления с помощью `views/03_create_views.sql`
5. Оптимизируйте производительность, добавив индексы из файла `indexes/indexes.sql`
6. Добавьте триггеры и ограничения из файла `triggers/triggers_constraints.sql`
7. Создайте хранимые процедуры из файла `procedures/stored_procedures.sql`
8. Выполните тестирование с помощью скрипта `utilities/tests.sql`
9. Изучите примеры использования в файле `utilities/examples.sql`
10. Выполняйте запросы к представлениям из файла `views/04_view_queries.sql`
11. Для проверки целостности данных выполните скрипт `data/02_sample_data.sql` до конца, он включает проверку количества записей в каждой таблице
...
├── triggers/         # Триггеры и ограничения
├── indexes/          # Индексы
