...
|:---:|:---|:---|
| **AC** | Accumulator | **Аккумулятор** — основной рабочий регистр процессора для выполнения арифметических и логических операций. В нем накапливаются промежуточные и конечные результаты вычислений. Все операции сложения, вычитания, логические операции выполняются с участием AC. Например, при команде `6108` содержимое ячейки 108 складывается с текущим значением AC, результат остается в AC. |
| **DR** | Data Register | **Регистр данных** — промежуточный буферный регистр для временного хранения данных при обмене между процессором и оперативной памятью. Когда процессор читает операнд из памяти, данные сначала загружаются в DR, затем передаются в AC или другие регистры. При записи в память данные из AC сначала копируются в DR, а потом записываются по указанному адресу. DR обеспечивает синхронизацию работы быстрого процессора и более медленной памяти. |
| **AR** | Address Register | **Регистр адреса** — хранит адрес ячейки памяти, к которой процессор обращается в текущий момент. При выполнении команды адресная часть команды (например, `108` в команде `6108`) загружается в AR, затем по этому адресу происходит чтение или запись данных. AR управляет шиной адреса и определяет, с какой ячейкой памяти работает процессор на данном шаге. |
| **PC** | Program Counter | **Счетчик команд** — специальный регистр, хранящий адрес следующей команды, которая должна быть выполнена. После выборки каждой команды PC автоматически увеличивается на 1, указывая на следующую команду в памяти. При командах перехода (например, `A109`) значение PC изменяется на адрес перехода. PC обеспечивает последовательное выполнение программы и управляет порядком исполнения команд. |
| **IR** | Instruction Register | **Регистр команд** — хранит текущую выполняемую команду, загруженную из памяти. После того как команда считывается из памяти по адресу, указанному в PC, она помещается в IR. Процессор декодирует содержимое IR, определяя код операции (например, `6` — сложение, `E` — сохранение) и адрес операнда. IR используется устройством управления для генерации управляющих сигналов, необходимых для выполнения команды. |
