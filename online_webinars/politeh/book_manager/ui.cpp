...
void UI::loggingMenu() {
    ...
    printf("  1. 📝 Включить логирование действий\n");
    printf("  2. 📋 Просмотреть историю действий\n");
    printf("  3. ⚙️  Установить максимальный размер истории\n");
    printf("  0. ← Назад\n");
    ...
    switch (choice) {
        case 1: {
            std::string filename = getStringInput("\nВведите имя файла для логирования (Enter для стандартного): ");
            if (filename.empty()) {
                library.enableActionLogging();
            } else {
                library.enableActionLogging(filename);
            }
            break;
        }
        case 2: {
            library.printActionHistory();
            break;
        }
        case 3: {
            int maxSize = getIntInput("\nВведите максимальный размер истории (по умолчанию 50): ");
            library.setMaxHistorySize(maxSize);
            break;
        }
        case 0:
            break;
